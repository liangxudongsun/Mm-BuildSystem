...
namespace Mm_Budier
{
    ...
    {
        //Chunk坐标 
        public Vector3Int coordVectorInt;
        //Chunk内的方块位置（局部坐标）
        public HashSet<Vector3Int> cubeInChunkHashSet;
        //Chunk的父物体
        public GameObject chunkFatherObj;
    }
    /// <summary>
    ...
    public class ChunkSystem : MonoBehaviour
    {
        ...
        {
            return GridToChunkWithLocal(gridPos).chunkCoord;
        }
        /// <summary>
        /// 网格坐标转 区块内相对坐标
        /// </summary>
        ...
        {
            return GridToChunkWithLocal(gridPos).localPos;
        }

        /// <summary>
        /// 网格坐标转 区块坐标 和 区块内相对坐标 
        /// 例如：网格坐标(17, 0, 17) → 区块坐标(1, 0, 1) 区块内相对坐标(1, 0, 1)
        /// 网格坐标(-1, 0, -1) → 区块坐标(-1, 0, -1) 区块内相对坐标(15, 0, 15)
        /// </summary>
        /// <param name="gridPos">网格坐标</param>
        /// <returns>区块坐标和区块内相对坐标</returns>
        public (Vector3Int chunkCoord, Vector3Int localPos) GridToChunkWithLocal(Vector3Int gridPos)
        {
            // 必须用浮点除法再 Floor，int 除法会向 0 截断，负数会算错
            ...
            // 区块内相对坐标 = 网格坐标 - 区块起始坐标(区块坐标 * 区块大小)
            // gridPos 为负数的时候 反向映射 比如 (-1, 0, -1) → (15, 0, 15)
            int lx = gridPos.x - cx * chunkSize;
            int ly = gridPos.y - cy * chunkSize;
            int lz = gridPos.z - cz * chunkSize;
        ...
        private void LoadChunk(Vector3Int coord)
        {
            ...
            {
                coordVectorInt = coord,
                cubeInChunkHashSet = new HashSet<Vector3Int>(),
                chunkFatherObj = new GameObject($"Chunk_{coord.x}_{coord.y}_{coord.z}")
            };
            chunk.chunkFatherObj.transform.SetParent(BuilderSystem.Instance.ChunkRoot);

            loadedChunks.Add(coord, chunk);
            // TODO: 从存储加载该Chunk的方块数据
        }
        ...
        private void UnloadChunk(Vector3Int coord)
        {
            ...
            {
                // TODO: 将该Chunk的方块数据保存到存储 
                if (chunk.chunkFatherObj != null)
                {
                    Destroy(chunk.chunkFatherObj);
                }
        ...
        public void AddCube(Vector3Int gridPos, CubeData cubeData)
        {
            ...
            if (loadedChunks.TryGetValue(chunkCoord, out Chunk chunk))
            {
                ...
                chunk.cubeInChunkHashSet.Add(localPos);
            }
        }
        ...
        public void RemoveCube(Vector3Int gridPos)
        {
            ...
            if (loadedChunks.TryGetValue(chunkCoord, out Chunk chunk))
            {
                chunk.cubeInChunkHashSet.Remove(localPos);
            }
        }
        ...
        /// <returns>区块内的方块数量</returns>
        public int GetChunkCubeCount(Vector3Int chunkCoord)
        {
            if (loadedChunks.TryGetValue(chunkCoord, out Chunk chunk))
            {
                return chunk.cubeInChunkHashSet.Count;
            }
            return 0;
        }
        #endregion
